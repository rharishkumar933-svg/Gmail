...
class MongoDB:
    ...
    async def create_user(self, user_id: int, username: str = None, first_name: str = None, referred_by: int = None) -> Dict:
        ...
        try:
            ...
            # If referred_by is valid and set, credit the referral reward directly to main balance
            if referred_by:
                referral_reward_inr = await self.get_system_setting("referral_reward_inr", 0.0)
                if referral_reward_inr > 0:
                    exchange_rate = await self.get_system_setting("exchange_rate", 100.0)
                    ...
                    await self.add_balance_transaction(
                        ...
                        details={
                            ...
                            "referred_username": username or f"User_{user_id}",
