...
async def stats_command(client: Client, message: Message):
    ...
    try:
        ...
        total_approved = await mongo.db["transactions"].count_documents({"status": "credited"})
        
        # Total Hold Accounts (hold transactions)
        total_hold = await mongo.db["transactions"].count_documents({"status": "hold"})
        
        # Total Rejected Accounts (rejected transactions)
        total_rejected = await mongo.db["transactions"].count_documents({"status": "rejected"})
        
        # Total Expired Accounts (expired transactions)
        total_expired = await mongo.db["transactions"].count_documents({"status": "expired"})
...
async def user_details_command(client: Client, message: Message):
    ...
    try:
        ...
        # Transaction Counts
        approved_count = await mongo.db["transactions"].count_documents({"user_id": target_user_id, "status": "credited"})
        hold_count = await mongo.db["transactions"].count_documents({"user_id": target_user_id, "status": "hold"})
        rejected_count = await mongo.db["transactions"].count_documents({"user_id": target_user_id, "status": "rejected"})
        expired_count = await mongo.db["transactions"].count_documents({"user_id": target_user_id, "status": "expired"})
...
@Client.on_message(filters.command("refferalamount") & filters.private)
@admin_only
async def set_referral_amount_command(client: Client, message: Message):
    """
    /refferalamount [amount_in_inr]
    Sets the referral reward in INR.
    """
    args = message.command
    if len(args) < 2:
        await message.reply("❌ **Usage:** `/refferalamount [amount_in_inr]`")
        return
    ...
    if amount_inr < 0:
        await message.reply("❌ **Referral amount cannot be negative.**")
        return
        
    mongo = client.mongo
    success = await mongo.set_system_setting("referral_reward_inr", amount_inr)
    
    if success:
        await message.reply(
            f"✅ **Referral reward successfully set to {amount_inr}₹ (INR).**"
        )
    else:
        await message.reply("❌ **Failed to update referral reward in database.**")
...
async def admin_commands_list_command(client: Client, message: Message):
    ...
    admin_help = (
        ...
        "• `/refferalamount [amount_inr]` - Set referral reward amount in INR\n"
