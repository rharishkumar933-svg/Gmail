...
def make_history_keyboard(transactions, total_count, page, per_page=5) -> InlineKeyboardMarkup:
    keyboard = []
...
@Client.on_message(filters.text & filters.private & ~filters.command(["start", "admin", "addbalance", "removebalance", "addholdbalance", "removeholdbalance", "waittime", "checkholds", "broadcast", "stats", "user", "payout", "refferalamount", "setup", "minimumpayout", "admincommands", "login", "margin", "setmargin"]))
...
Hold funds will automatically be transferred to your Main Balance after the hold period expires."""
        ...
        if not transactions:
            ...
            if total_pages == 0:
                total_pages = 1
            reply_markup = make_history_keyboard(transactions, total, 1)
            exchange_rate = await mongo.get_system_setting("exchange_rate", 100.0)
...
![3️⃣](tg://emoji?id=6035205087045884098) Submit the account details back to the bot and get paid instantly! ![🚀](tg://emoji?id=6021813997492246416)"""
        ...
        ref_text = (
            ...
            f"💰 **Referral Reward:** {reward_str} per referral\n\n"
...
async def tx_page_callback(client: Client, callback: CallbackQuery):
    ...
    try:
        ...
        if total_pages == 0:
            total_pages = 1
        reply_markup = make_history_keyboard(transactions, total, page)
        exchange_rate = await mongo.get_system_setting("exchange_rate", 100.0)
